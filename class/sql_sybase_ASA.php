...
//============================================ menu ==================================================
	...
	//function to get menu
	public function menuData($menuParent,$filter)
	{
		//if have filter		
		if(count($filter) > 0)
		{
			if(array_key_exists('parentmenu',$filter))
			{
				if($filter['parentmenu'] == '-')
					$extraMenu=" and b.MENUPARENT = 0";
				else
					$extraMenu=" and upper(b.MENUNAME) like upper('%".$filter['parentmenu']."%')";
			}
			
			if(array_key_exists('title',$filter))
				$extraParent=" and upper(a.MENUNAME) like upper('%".$filter['title']."%')";
		}
...
//========================================= lov editor ===============================================
	//get menu for lov editor
	public function getLovActiveMenu($filter='')
	{
		$sql = "select * from (
						select concat(a.MENULINK,'&menuID=',a.MENUID) FLC_ID,
							concat(if(isnull(c.MENUNAME),'',concat(c.MENUNAME ,' / ')),b.MENUNAME ,' / ' , a.MENUNAME) as FLC_NAME
						from FLC_PAGE d, FLC_MENU a
							left outer join FLC_MENU b on a.MENUPARENT = b.MENUID 
							left outer join FLC_MENU c on b.MENUPARENT = c.MENUID
						where a.MENUID = d.MENUID and a.MENUPARENT != 0
						UNION
						select 'login.php', 'Login'
						UNION
						select 'index.php?logout=true', 'Logout'
					) a 
					where FLC_ID like upper('%".$filter."%') or FLC_NAME like upper('%".$filter."%')
					order by 2";
		return $this->query($sql,'SELECT','NAME');
	}//eof function
//======================================= eof lov editor =============================================
