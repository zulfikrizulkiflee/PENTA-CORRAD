...
{	
	...
	{	
		...
		{	
			...
			$this->dbc = @sybase_connect($this->dbConnection,$this->dbUsername,$this->dbPassword,'utf8');

			//if not connected, show error msg and return false
			if($this->dbc == FALSE)
			{	
				sqlErrorLog(sybase_get_last_message());
	...
	{	
		//select db
		$selected = @sybase_select_db($this->dbName,$this->dbc);
...
{	
	...
	private function run_sql($sql)
	{
		...
		@sybase_query('set char_convert on',$this->dbc); 
		@sybase_query('set char_convert "utf8"',$this->dbc); 
		
		return @sybase_query($sql,$this->dbc) or sqlErrorLog(sybase_get_last_message(), $sql);
		//return @sybase_query($sql,$this->dbc) or die('DATABASE INSERT, UPDATE, DELETE QUERY ERROR: '.$sql);	//run query
	}
	...
	{
		$sqlRs = @sybase_query($sql,$this->dbc) or sqlErrorLog(sybase_get_last_message(), $sql);
		//$sqlRs = @sybase_query($sql,$this->dbc) or die('DATABASE BOOLEAN QUERY ERROR ');	//run query
		
		//if query success
		if($sqlRs != FALSE)
		{	
			$sqlNumRows = @sybase_num_rows($sqlRs);		//num rows 
	...
	{
		$sqlRs = @sybase_query($sql,$this->dbc) or sqlErrorLog(sybase_get_last_message(), $sql);
		//$sqlRs = @sybase_query($sql,$this->dbc) or die('DATABASE COUNT QUERY ERROR ');	//run query
		
		//if query success		
		if($sqlRs != FALSE)
		{
			$sqlRsRows = sybase_fetch_array($sqlRs);
	...
	{
		$sqlRs = @sybase_query($sql,$this->dbc) or sqlErrorLog(sybase_get_last_message(), $sql);
		//$sqlRs = @sybase_query($sql,$this->dbc) or die('DATABASE SELECT QUERY ERROR '.$sql);	//run query
		...
		{	
			...
			if($sqlNumRows != 0)
			{
				...
				for($x=0; $x < $sqlNumRows; $x++)
				{
					...
					$tempArray[] = sybase_fetch_array($sqlRs);
				}
				...
				sybase_free_result($sqlRs);					//free result memory
	...
	{
		sybase_close() ; // to close the opened sql
	}
	//-----------------------------------------
	//[ end class methods declaration block ]
	//-----------------------------------------
