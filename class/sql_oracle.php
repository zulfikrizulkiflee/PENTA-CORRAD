...
class dbSQLOracle extends dbSQLBatch
{
	protected $dbQuery;
	protected $dbc;
...
//========================================== database ==============================================
	...
	public function listTableColumn($schema,$user,$table,$columnExclude='',$addAttr=array(),$order='COLUMN_NAME')
	{
		...
		$sql="select upper(COLUMN_NAME) COLUMN_NAME ".$addAttrStr."
	...
	public function columnDatatype($schema,$user,$table,$column='',$extraInfo=array())
	{
		...
		if(count($extraInfo))
		{
			...
			{
				$extraInfoCol .= ', a.data_length as "SIZE"';
			}
			if(in_array('primary',$extraInfo))
			{
				$extraInfoCol .= ', decode(a.column_name, b.column_name,\'PRI\') as PRI_KEY';
			}
		...
		if($column)
			$extra=" and upper(a.COLUMN_NAME) = upper('".$column."')";
		...
		$sql="select upper(a.COLUMN_NAME) COLUMN_NAME, upper(a.DATA_TYPE) DATA_TYPE ".$extraInfoCol."
				...
				and upper(a.OWNER) in ('".implode("','",explode(',',strtoupper($user)))."')".$extra."
	...
	public function storeUnlimitedChar($tableName, $columnName, $unlimitedChar, $whereClause='')
	{
		...
		$stmt = oci_parse($this->dbc, $sql);
		oci_bind_by_name($stmt, ":unlimitedChar", $unlimitedChar, -1);
...
//======================================= date functions ===========================================
	...
	{
		return "to_char(".$datetime.",'".$this->date_format()." HH24:MI:SS')";
	}//eof function
	...
	{
		return "to_date('".$datetime."','".$this->date_format()." HH24:MI:SS')";
	}//eof function
//===================================== eof date functions ===========================================
