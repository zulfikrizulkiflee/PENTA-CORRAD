...
//============================================ menu =================================================
	...
	/*public function getMenuList($parentId='', $permissionFlag=true)
	{
		//root menu
		...
		//get list of menu
		$getMenuList = "select MENUID, MENUNAME, MENUTITLE, MENUPARENT, MENULINK, MENUTARGET, MENUORDER, MENUSTATUS, MENUHINTS, MENULEVEL, MENUSTATE, MENUICON
							from FLC_MENU where MENUPARENT = ".$parentId." ".$menuPermissionSQL." order by MENUORDER";
		...
		for($x=0; $x<$getMenuListRsCount; $x++)
		{
			...
			$getMenuListRs[$x]['MENUCHILD'] = $this->getMenuList($getMenuListRs[$x]['MENUID'], $permissionFlag);
		}//eof for
	...
	public function getMenuIdByMenuParent($menuId, $includeParent = true)
	{
		...
		if($menuId)
		{
			...
			for($x=0; $x<$getMenuIdRsCount; $x++)
			{
				...
				//informix version
				$relatedMenuId[] = $getMenuIdRs[$x]['MENUID'];
